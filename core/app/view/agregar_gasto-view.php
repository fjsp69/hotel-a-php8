...
<!-- LO QUE QUEDA -->
        ...
        <?php $efectivo=0; $tipos1 = TipoPagoData::getAll();
          ...
          <?php $efectivo_total=0; ?>
          <?php $efectivo_total = $caja_abierta->monto_apertura + $efectivo; 
...
if(@count($_POST)>0 and $_POST["precio"]<=$efectivo_total){
...
print "<script>window.location='index.php?view=imprimir_gasto&id=$id';</script>";

}else{
print "<script>alert('No hay suficiennte dinero en caja para procesar el gasto');</script>"; 
print "<script>window.location='index.php?view=egreso';</script>";
}
